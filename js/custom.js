(function($) {
  ...
  var localCalendar = $('#local-calendar');
  ...
  $.getJSON(finalURL, function (data) {
    ...
    localCalendar.html(output);
  });

  // -- news feed
  var newsURL = '/news.json?start_date=' + now;

  var groupNews = $('#group-news');

  $.getJSON(newsURL, function (data) {
    var output = "";
    var newsCount = 0;
    for (let i in data.results) {
      if (moment(data.results[i].date, dateFormat).diff(now, 'days') >= 0 ) {
        newsCount++;
